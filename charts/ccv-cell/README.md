...
| aggregator.grpcRoute | object | `{"annotations":{},"enabled":false,"hostnames":[],"labels":{},"parentRefs":[]}` | Enable a Gateway API GRPCRoute for the aggregator gRPC endpoint. **Preferred over `httpRoute`** when your    gateway controller supports it — GRPCRoute is the purpose-built Gateway API resource for gRPC traffic    (method-aware routing, gRPC-status-aware filters), whereas HTTPRoute has to treat gRPC as opaque HTTP/2.    GRPCRoute only reached GA in Gateway API v1.1 (later than HTTPRoute), so controller support is narrower —    `httpRoute` is provided as a fallback for controllers that don't yet implement GRPCRoute. Mutually    independent of `ingress.enabled` and `httpRoute.enabled`; enable whichever your controller actually supports. |
